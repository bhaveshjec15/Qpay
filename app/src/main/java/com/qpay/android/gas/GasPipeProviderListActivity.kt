...
import com.qpay.android.databinding.ActivityGasPipeProviderListBinding
import com.qpay.android.gas_cylinder.GasCynProviderListAdapter
import com.qpay.android.gas_cylinder.GasCynProviderListModel
...
class GasPipeProviderListActivity : AppCompatActivity() {
  private lateinit var binding: ActivityGasPipeProviderListBinding
  var list: ArrayList<GasPipeProviderListModel> = ArrayList()
  var listTemp: ArrayList<GasPipeProviderListModel> = ArrayList()
  var adapter: GasPipeProviderListAdapter? = null
  var adapterBanner: SliderNewAdapter? = null
  private var sliderDataArrayList: ArrayList<SliderData>? = null

  override fun onCreate(savedInstanceState: Bundle?) {
    super.onCreate(savedInstanceState)
    binding = DataBindingUtil.setContentView(this,R.layout.activity_gas_pipe_provider_list)
    ...
    var param = FasTagBillerRequest(resources.getString(string.param_gas_pipe), 0, 0)
    callApi(param, this)
    ...
    binding.etSearch.addTextChangedListener(object : TextWatcher {
      ...
      override fun afterTextChanged(s: Editable) {
        ...
        if(list.size>0){
          ...
          if(listTemp.size>0){
            adapter = GasPipeProviderListAdapter(baseContext, listTemp)
  ...
  private fun callApi(param: FasTagBillerRequest, gasPipeProviderListActivity: GasPipeProviderListActivity) {
    ...
    apiInterface.enqueue(object : Callback<ResponseBody> {
      override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
        ...
        if (statusCode == 200) {
          ...
          for (i in 0..ss) {
            var model = GasPipeProviderListModel()
          ...
          if(list.size>0){
            adapter = GasPipeProviderListAdapter(gasPipeProviderListActivity, list)
            binding.rvBillerList.adapter = adapter
            adapter?.notifyDataSetChanged()
          }
        } else {
          showSnackBar(gasPipeProviderListActivity, binding.mainLayout, message)
        }
