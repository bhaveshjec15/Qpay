...
import com.qpay.android.databinding.ActivityProviderListBinding
import com.qpay.android.fasTag.FasTagBillerListModel
import com.qpay.android.fasTag.FasTagListAdapter
...
class ProviderListActivity : AppCompatActivity() {
  ...
  var list: ArrayList<ProviderListModel> = ArrayList()
  var listTemp: ArrayList<ProviderListModel> = ArrayList()
  var adapter: ProviderListAdapter? = null
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    var param = FasTagBillerRequest(resources.getString(R.string.param_dth), 0, 0)
    callApi(param, this)
    ...
    binding.etSearch.addTextChangedListener(object : TextWatcher {
      ...
      override fun afterTextChanged(s: Editable) {
        ...
        if(list.size>0){
          ...
          if(listTemp.size>0){
            adapter = ProviderListAdapter(baseContext, listTemp)
  ...
  private fun callApi(param: FasTagBillerRequest, providerListActivity: ProviderListActivity) {
    ...
    apiInterface.enqueue(object : Callback<ResponseBody> {
      override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
        ...
        if (statusCode == 200) {
          ...
          if(list.size>0){
            adapter = ProviderListAdapter(providerListActivity, list)
            binding.rvBillerList.adapter = adapter
            adapter?.notifyDataSetChanged()
          }
        } else {
          showSnackBar(providerListActivity, binding.mainLayout, message)
        }
