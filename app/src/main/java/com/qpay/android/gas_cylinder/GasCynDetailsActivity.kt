...
class GasCynDetailsActivity : AppCompatActivity() {
  private lateinit var binding: ActivityGasCynDetailsBinding
  var billerId: String? = ""
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    binding = DataBindingUtil.setContentView(this, R.layout.activity_gas_cyn_details)
   ...
   // createAllView()
    ...
    binding.btnSubmit.setOnClickListener {
      ...
      if (fetchOption.equals("MANDATORY", true)) {
        ...
        if(binding.etAmountOne.text.toString().isNullOrEmpty()){
          showSnackBar(this, binding.mainLayout, "Please Enter Mobile Number")
        }
        else{

          var paramsMain: HashMap<String, String> = HashMap()
          paramsMain.put("Mobile Number", binding.etAmountOne.text.toString())
  ...
  private fun callApiFetchBill(param: GasCynFetchBillRequest, gasCynDetailsActivity: GasCynDetailsActivity) {
    ...
    apiInterface.enqueue(object : Callback<ResponseBody> {
      override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
        ...
        if (statusCode == 200) {
          ...
          val intent = Intent(gasCynDetailsActivity, GasCynBillPayActivity::class.java)
          ...
          intent.putExtra("number",additionalParams.optString("Consumer Number"))
          ...
          intent.putExtra("mobile_number",binding.etAmountOne.text.toString())
          startActivity(intent)
        }
        else {
          showSnackBar(gasCynDetailsActivity, binding.mainLayout, message)
        }
